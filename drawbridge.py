...
#
# In this model, of single deck drawbridge design, there is 1 bridge deck of
# length (500 feet/152.4 m). The deck is attached to the first pylon with 
# either a joint or a motor depending on whether the user wants to run a
# quasi-static or kinematic analysis.
###############################################################################
...
vis.SetWindowTitle('draw bridge')
vis.Initialize()
vis.AddSkyBox()
...
ax2.plot(array_time,array_1t)
ax2.set(ylabel='Reaction Torque [Nm]',xlabel='Time [s]')
ax2.grid()

if mj == "pseudo-static":
    plt.savefig('draw bridge - pseudo-static.png')
    plt.xlim(0.2,14.8)
    plt.show()
elif mj == "kinematic":
    plt.savefig('draw bridge - kinematic.png')
    plt.xlim(0.2,14.8)
    plt.show()
else:
    plt.savefig('draw bridge - unknown.png')
    plt.xlim(0.2,14.8)
    plt.show()
