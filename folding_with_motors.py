...
motor2.Initialize(deck1, deck2, chrono.ChFrameD(chrono.ChVectorD(-l/6, 10, 5), z2x))
motor2.SetAngleFunction(chrono.ChFunction_Ramp(0, omg))
system.AddLink(motor2)

motor3 = chrono.ChLinkMotorRotationAngle()
motor3.Initialize(deck2, deck3, chrono.ChFrameD(chrono.ChVectorD(l/6, 10, 5), z2x))
motor3.SetAngleFunction(chrono.ChFunction_Ramp(0, -omg))
system.AddLink(motor3)
...
while (vis.Run() and system.GetChTime() < time_end):
    ...
    array_3x.append(motor3.Get_react_force().x)
    array_3y.append(motor3.Get_react_force().y)
    array_3z.append(motor3.Get_react_force().z)
    array_3t.append(motor3.GetMotorTorque())
...
ax4.plot(array_time,array_2t)
ax4.set(ylabel='Reaction Torque [Nm]')
ax4.grid()

plt.savefig('folding results - torque.png')
plt.show()
