...
deck2.SetPos(chrono.ChVectorD(l/3,10+((d)/2),5))
...
deck3.SetMass(mass)
deck3.SetIdentifier(4)
deck3.SetPos(chrono.ChVectorD(0,10+((d)/2),5))
...
joint2.Initialize(deck1,
                   deck2, 
                   chrono.ChCoordsysD(chrono.ChVectorD(-l/6, 10, 5)))
system.Add(joint2)

joint3 = chrono.ChLinkLockRevolute()
joint3.Initialize(deck2,
                   deck3, 
                   chrono.ChCoordsysD(chrono.ChVectorD(l/6, 10, 5)))
system.Add(joint3)
...
while (vis.Run() and system.GetChTime() < time_end):
    ...
    array_1x.append(joint2.Get_react_force().x)
    array_1y.append(joint2.Get_react_force().y)
    array_1z.append(joint2.Get_react_force().z)
    array_2x.append(joint1.Get_react_force().x)
    array_2y.append(joint1.Get_react_force().y)
    array_2z.append(joint1.Get_react_force().z)
    array_3x.append(joint1.Get_react_force().x)
    array_3y.append(joint1.Get_react_force().y)
    array_3z.append(joint1.Get_react_force().z)
